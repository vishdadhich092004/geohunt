...
export const ClassicSkeleton = ({ mode }: { mode: GameModeType }) => {
  return (
    <motion.div
      initial="initial"
      whileHover="animate"
      className="flex flex-1 w-full h-full min-h-[6rem] rounded-lg p-4 flex-col space-y-4"
      style={{
        backgroundImage:
    ...
    >
      <div className="flex items-center justify-between">
        <div className="h-8 w-8 rounded-full bg-primary text-amber-400 flex items-center justify-center">
          {mode.maxLives}
        </div>
        <Heart className="h-6 w-6 text-amber-400" />
      </div>
      <div className="flex-grow flex items-center justify-center">
        <div className="text-2xl font-bold text-white">Classic Mode</div>
      </div>
    </motion.div>
  );
};
...
export const SpeedSkeleton = ({ mode }: { mode: GameModeType }) => {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={variants} className="h-2 bg-primary rounded-full" />
    </motion.div>
  );
};
...
export const HardcoreSkeleton = () => {
  ...
  return (
    <motion.div
      initial="initial"
      whileHover="animate"
      className="flex flex-1 w-full h-full min-h-[6rem] rounded-lg p-4"
      style={{
        backgroundImage:
